...
#定义hosts
declare -A map=(["192.168.1.21"]="cdh1" ["192.168.1.22"]="cdh2" ["192.168.1.23"]="cdh3")
#定义主节点
master=192.168.1.21
#定义网关
getway=192.168.1.10
...
#注意：脚本命令修改mysql密码必须指定--connect-expired-password参数(坑惨了)
/usr/bin/mysql -uroot -p${psw} --connect-expired-password -e 'source /root/c.sql'
#mysql  -uroot -p${psw}  << EOF
#source /root/c.sql
#EOF
...
sha1sum /opt/cloudera/parcel-repo/CDH-6.3.0-1.cdh6.3.0.p0.1279813-el7.parcel | awk '{ print $1 }' > CDH-6.3.0-1.cdh6.3.0.p0.1279813-el7.parcel.sha
